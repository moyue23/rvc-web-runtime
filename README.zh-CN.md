...
| **模型格式**            | ✅ ONNX/PTH | 支持 `.onnx` 模型，`.pth` 自动转换（使用 rvc-onnx-web） |

### 🔄 进行中 / 计划

| 功能                      | 状态      | 说明                                      |
| ------------------------- | --------- | ----------------------------------------- |
| **特征检索（Retrieval）** | 🚧 计划中 | 基于 faiss 索引的特征替换，提升音色相似度 |
| **WebGPU 加速**           | 🚧 计划中 | 当前 WASM 后端；WebGPU 加速推理           |
| **音量包络匹配**          | 🚧 计划中 | RMS mix rate，更自然的音量过渡            |
| **UV（无声）处理**        | 🚧 计划中 | 更好的呼吸音和静音处理                    |
